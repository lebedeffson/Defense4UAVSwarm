...
CHECKS = [f"C{i:02d}" for i in range(1, 46)]
...
def main() -> None:
    ...
    checks: dict[str, tuple[str, str]] = {
        ...
        "C20": check(not curves.empty and "median_gate_delay" in curves.columns, "gate metadata/delay columns are present"),
        "C21": check(not curves.empty and curves["median_gate_delay"].fillna(0).ge(0).all(), "confirmation delays are non-negative"),
        ...
        "C28": check(not loso.empty and loso.get("outer_fold", pd.Series(dtype=int)).nunique() == 7, "outer folds = 7 and test sequence unique"),
        ...
        "C35": check(not curves.empty and "label_access" in curves.columns, "label-access metadata filled"),
        "C36": check(not label.empty, "label-scarcity corrected run completed"),
        "C37": check(not runtime.empty and "evaluator_offline_excluded" in runtime.columns, "runtime benchmark separated from evaluator"),
        ...
        "C40": check(len(set(c for c in commits if c)) == 1 and bool(commits), "manifest commit equals run commit"),
    ...
    df.to_csv(out / "closure_checks_c01_c45.csv", index=False)
    failed = df[df["status"].eq("FAIL")]
    status = "PASS" if failed.empty else "FAIL"
    report = ["# v5.4.2 Article-Grade Closure C01-C45", "", df.to_string(index=False), ""]
    report.append("ARTICLE_READY_PASS" if failed.empty else "DO_NOT_TRANSFER_TO_ARTICLE")
    (out / "closure_checks_report.md").write_text("\n".join(report) + "\n", encoding="utf-8")
    print(f"status={status} output={out / 'closure_checks_c01_c45.csv'} failed={len(failed)}")
    if not failed.empty:
        sys.exit(1)
