...
def main() -> None:
    ...
    if args.feature_audit:
        clean = pd.read_csv(args.feature_audit)
        gt = load_gt(args.dataset_root, sorted(clean["sequence_id"].unique()) if not clean.empty else None)
        keys = clean[["sequence_id", "frame_id"]].drop_duplicates()
        gt = gt.merge(keys, on=["sequence_id", "frame_id"], how="inner")
    else:
        detections = args.detections or infer_detections_path(args.model)
        clean_raw = load_detections(detections)
...
def infer_detections_path(model: str) -> str:
    stem = Path(model).stem if model else "yolov8n"
    return f"outputs/results/q1_real_detector/detections/{stem}_visdrone.json"
