...
def main() -> None:
                                      ...
                                      for tau_new in tau_new_values:
                                       ...
                                       for confirm_age in confirm_age_values:
                                        for confirmed_conf_floor in confirmed_floors:
                                         tau = tau_existing
                                         spec_key = (t_norm, tau, k_variant, alpha_scale, gamma_assoc, filter_mode, soft_conf_floor, reject_patience, beta, tau_existing, tau_new, confirm_age, confirmed_conf_floor)
                                         clean = apply_tnorm(
                                            s0,
                                            t_norm,
                                            tau,
    ...
    payload = {
        ...
        "reason": None if selection_status != "not_selected" else "tracking_constraints_failed",
        "selected": {
            args.model: {
                f"eps_{args.eps}": {
...
def finalize_tracking_selection(table: pd.DataFrame, err: pd.DataFrame, cfg: dict, gt: pd.DataFrame) -> tuple[pd.DataFrame, dict, str]:
    ...
    for optional in ["tau_existing", "tau_new", "confirm_age", "confirmed_conf_floor", "reject_patience", "beta"]:
    ...
    if status == "not_selected":
        out.loc[:, "reason"] = "tracking_constraints_failed"
