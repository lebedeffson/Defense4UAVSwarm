...
def terminalize_initiation(
    ...
    for _, group in candidates.sort_values(["sequence_id", "tracklet_id", "frame_id"]).groupby(["sequence_id", "tracklet_id"], sort=False):
        confirmed = False
        rejected = False
        first_frame = None
        last_frame = None
        for idx, row in group.sort_values("frame_id").iterrows():
            ...
            if first_frame is None:
                first_frame = frame
            if last_frame is not None and frame - last_frame > cfg.max_track_gap + 1 and not confirmed:
                rejected = True
            last_frame = frame
